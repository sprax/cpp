// wordRectOld.cpp : Defines the entry point for the console application.
#include "stdafx.h"
...
static int findWordRectRowsTrie(  int wantWide, int wantTall, int haveTall, TrieSL tries[]
    , const char *wordRows[MAXLEN], char wordCols[MAXLEN][MAXLEN])
{
    //TotalCalls++;
    if (haveTall == wantTall) {
...
static int findWordRectRowsUsingGetNextWordNodeAndIndex(  int wantWide, int wantTall, int haveTall, TrieSL& rowTri, TrieSL& colTri
    , const char *wordRows[MAXLEN], const TrieSln ** colNodes[BUFSIZE] )
{
    //TotalCalls++;
    if (haveTall == wantTall) {
        printWordRect(wordRows, 1, haveTall);
...
static int findWordRectRowsTriS(  int wantWide, int wantTall, int haveTall, TrieSL& rowTri, TrieSL& colTri
    , const char *wordRows[MAXLEN], const TrieSln ** colNodes[BUFSIZE] )
{
    //TotalCalls++;
    if (haveTall == wantTall) {
    ...
    const TrieSln *rowNodesMem[BUFSIZE], **rowNodes = &rowNodesMem[1];  // put row nodes on the local stack: 1 row per stack level.
    ...
    for (int k = 0; rowWordNode != NULL; ) {
        ...
        for (;;) {
            ...
            const TrieSln *colBranch = colNodes[k][prevTall]->getBranchAtLetter(word[k]);   // Need this later only if non-NULL.
            if (colBranch == NULL) {
        ...
        // disqualified node, as does TrieSL::getNextWordNodeAndIndex, but over all, that is slower.)
        const TrieSln * nextStem = rowNodes[k]->getNextStemNode();
        if (nextStem == NULL) {
...
{
    TrieSL tries[BUFSIZE];
    MapSL   maps[BUFSIZE];
    char  ctimes[BUFSIZE];
    
    initFromSortedFile(tries, maps, "WORD.LST.txt");	// call this once; word/trie/map memory is retained for life of the program.
    
    int area = 0, haveArea = 0, wantArea = 0;
    const char **wordRows = WordRows;
    char prevCols[MAXLEN][MAXLEN];
    const TrieSln  *colNodesMem[BUFSIZE][BUFSIZE],   **colNodes[BUFSIZE];
    ...
    time_t timeBeg;
    time( &timeBeg );
    ctime_s(ctimes, BUFSIZE, &timeBeg );
...
int test_wordRectOld(int argc, char* argv[])
{
    int  minArea = 4;
    int  minTall = 2;
    int  maxTall = MAXLEN;
