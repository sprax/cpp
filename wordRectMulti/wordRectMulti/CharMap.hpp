...
public: 
	...
	typedef enum { 
		...
		eIdentityCharMap    =  1,   // Uses unsigned chars themselves as indexes
		eCompactCharMap     =  2,	// Uses an array to to maps unsigned chars to contiguous range of indices
		eFreqFirstCharMap   =  3,	// Uses an array to to maps unsigned chars to contiguous range of indices
		eEnglishRoman52Map  =  4,
		eEnglishLower26Map  =  5,
		eEnglishUpper26Map  =  6,
	} SubType;
    ...
    static const char *getSubTypeName(SubType st) {
        ...
        case eFreqFirstCharMap:     return "MostFrequentFirst";
	...
	inline uint targetSize()        const { return mTargetSize;     }
	inline uint targetOffset()      const { return mTargetOffset;   }
    inline uchr sourceMinChar()     const { return mSourceMinChar;  }
    inline uchr sourceMaxChar()     const { return mSourceMaxChar;  }

protected:
	CharMap(uint size, uint offset, uchr minChar, uchr maxChar)	
        : mTargetSize(size), mTargetOffset(offset)
        , mSourceMinChar(minChar), mSourceMaxChar(maxChar)
    { }

private:
	uint  mTargetSize;		// Size of the character-indexed output range.
	uint  mTargetOffset;	// Index of first entry == Distance from mBranches to &mBranches[charIndex(mMinChar)]
    uchr  mSourceMinChar;
    uchr  mSourceMaxChar;
};
...
{
public:
	IdentCharMap(uint size, uint offset, uchr minChar, uchr maxChar) 
        : CharMap(size, offset, minChar, maxChar) { }
...
{
public:
	inline	uint	charToIndex(uchr uc)  const { return mCharToIndexPtr[uc]; };

protected:
	ArrayCharMap(uint size, uint offset, uchr minChar, uchr maxChar) 
		:  CharMap(size, offset, minChar, maxChar) 
	{ }
...
public:
	...
	static SubType subType()	{ return eFreqFirstCharMap; }
};
