...
template <typename T>
typename WordRectSearchMgr<T>::FinderThreadInfo *WordRectSearchMgr<T>::sFinders[sHardMaxFinders];

template <typename T>	int		WordRectSearchMgr<T>::sVerbosity		= 0;
template <typename T>	int		WordRectSearchMgr<T>::sNumFinders		= 0;
template <typename T>	int     WordRectSearchMgr<T>::sSoftMaxFinders   = 0;
template <typename T>	uint    WordRectSearchMgr<T>::sFinderOptions	= 0;
template <typename T>	time_t  WordRectSearchMgr<T>::sSearchStartTime	= 0;
template <typename T>	int		WordRectSearchMgr<T>::sFoundArea		= 0;
template <typename T>	int		WordRectSearchMgr<T>::sMinimumArea		= 0;

#ifndef _MBCS
template <typename T>
static HANDLE CreateThread(HANDLE handle, int notUsed, typename WordRectSearchMgr<T>::finderThreadFnType finder, void *pArgs, int notB, DWORD *threadId)
{
	finder(pArgs);
	return (HANDLE) 1;
}
...
template <typename T> 
int WordRectSearchMgr<T>::manageSearch(uint minArea, uint minTall, uint maxTall, uint maxArea, uint maxWordLength)
{
	...
	if (mSingleThreaded) {
		sSoftMaxFinders = 1;
		printf("Single-threaded mode: Searching with only one thread.\n");
	} else {
		SYSTEM_INFO sysinfo;
		...
		printf("Found %d processors (or cores); will search with up to %d threads.\n", numProcessors, sSoftMaxFinders);
	}
	...
	ctime_safe(ctimeBuf, BUFSIZE, &sSearchStartTime );
	...
	while( nextWantWideTall(wantWide, wantTall, minTall, maxTall, minArea, maxArea, maxWordLength, mAscending) ) {
		...
		assert(mWordTries);
		WordRectFinder<T> *pWRF = NULL;
		if (mFindLattices) {
			pWRF = new WordLatticeFinder<T>(wantWide, wantTall, sFinderOptions, mWordTries, mWordMaps);
		} else if (mFindWaffles) {
			pWRF = new WordWaffleFinder<T>(wantWide, wantTall, sFinderOptions, mWordTries, mWordMaps);
		} else {
			pWRF = new WordRectFinder<T>(wantWide, wantTall, sFinderOptions, mWordTries, mWordMaps);
		}
#ifdef _MBCS
		HANDLE threadHandle = CreateThread   ( NULL, 0, (LPTHREAD_START_ROUTINE)finderThreadFunc, pWRF, 0, &threadId );
#else
		HANDLE threadHandle = CreateThread<T>( NULL, 0, (LPTHREAD_START_ROUTINE)finderThreadFunc, pWRF, 0, &threadId );
#endif
	...
	ctime_safe(ctimeBuf, BUFSIZE, &timeEnd );
...
template <typename T> 
unsigned long WordRectSearchMgr<T>::finderThreadFunc( void *pvArgs )
{
	WordRectFinder<T> *pWRF = static_cast<WordRectFinder<T> *>(pvArgs);
	...
	const int begFoundArea =  getFoundArea();
    ...
    if (doRegister) {
        ...
        if (sVerbosity) {
            printf("CANCEL %2d * %2d because %3d <= %3d (thread %2d dying after 0 seconds)\n"
                , wantWide, wantTall, wantArea, begFoundArea, sNumFinders);
        }
    }
	...
	if (doRegister && wantArea > getFoundArea() ) {
		area = pWRF->doSearch();
	}
	...
	{   //beg CriticalSection block
		...
		// Get latest max area found by any finder thread.
		// If this one found the new max area, save it.
		int endFoundArea = getFoundArea();
...
#define MSG_LEN 80
            ...
            if (pWRF->getState() == WordRectFinder<T>::eAborted && sVerbosity > 1) {
                sprintf_safe(statusMsg, MSG_LEN, "Aborted, %d rects, because %d <= %d.", numFound, area, endFoundArea);
            } else if (numFound > 0) {
            ...
            if (area > 0 /* && ! (sFinderOptions & WordRectFinder<T>::eFindAll) */ ) {
	...
	if (result == 0) {
		printf("Error from ReleaseSemaphore: res:%d.  Aborting finderThreadFunc.\n", result, GetLastError() );
		return 7;
	}
	return 0;
}
...
template <typename T> 
void WordRectSearchMgr<T>::sigintHandler ( int sig )
{
	...
	char    ctimeBuf[ BUFSIZE ];
	ctime_safe(ctimeBuf, BUFSIZE, &timeNow );
...

	{
		...
		for (int j = 0; j < sNumFinders; j++) {
			...
			if (pFTI) {
				const WordRectFinder<T> *pWRF = pFTI->getFinder();
...
template <typename T>
int WordRectSearchMgr<T>::nextWantWideTall(int& wide, int& tall, const int& minTall, const int& maxTall
...
template <typename T> 
void WordRectSearchMgr<T>::printWordRectMutex(WordRectFinder<T> *pWRF)
{
	if (sVerbosity > 0) {
		time_t timeNow ;
		time( &timeNow );
...
//template <typename T> 
//void WordRectSearchMgr<T>::printFinderWordWaffle(WordRectFinder<T> *pWRF)
//{
//	time_t timeNow ;
//	time( &timeNow );
//	EnterCriticalSection(&gcsFinderSection);
