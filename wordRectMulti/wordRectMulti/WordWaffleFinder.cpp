...
template <typename T>
void WordWaffleFinder<T>::initRowsAndCols()
{
    if (this->mRowWordsNow != NULL)
        return;

    this->mRowWordsNow = (const char **)malloc(this->mWantTall * sizeof(char *));
    this->mRowWordsOld = (const char **)malloc(this->mWantTall * sizeof(char *));
    this->mRowNodes    = (const TrieNode ***)malloc(this->mWantTall * sizeof(TrieNode **));
    this->mColNodes    = (const TrieNode ***)malloc(this->mWantWide * sizeof(TrieNode **));
    ...
    this->mRowNodesMem = (const TrieNode  **)malloc(wideP1 * this->mWantTall * sizeof(TrieNode *));
    ...
    this->mColNodesMem = (const TrieNode  **)malloc(this->mWantWide * tallP1 * sizeof(TrieNode *));
...
template <typename T>
int WordWaffleFinder<T>::findWordRows()
{
...
template <typename T>
int WordWaffleFinder<T>::findWordRowsUsingTrieLinks(int haveTall)
{
    ...
    if (haveTall == 0) {
        ...
        if (haveTall % 2 == 0) {
            rowWordNode = this->mEvnRowTrie.getFirstWordNodeFromIndex(rowCharIndex(this->mColNodes[0][prevTall]->getLetterAt(haveTall)));
        } else {
            rowWordNode = this->mOddRowTrie.getFirstWordNodeFromIndex(rowCharIndex(this->mColNodes[0][prevTall]->getLetterAt(haveTall)));
            evenRow = false;
            lastCol = mOddWide - 1;
        }
    ...
    */
    const char *rowWord;
    const TrieNode **rowNodes = this->mRowNodes[haveTall], *colBranch, *nextStem;
    for (int col = 0; rowWordNode != NULL; ) {
        rowWord = rowWordNode->getWord();
        for (;;) {
...
template <typename T>
bool WordWaffleFinder<T>::isSymmetricSquare() const
{
...
template <typename T>
void WordWaffleFinder<T>::printWordRows(const char *wordRows[], int haveTall) const
{
    for (int row = 0; row < haveTall; ++row) {
