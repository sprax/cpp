...
template <typename T>
int WordWaffleFinder<T>::findWordRowsUsingTrieLinksA(int)
{
...
*/
template <typename T>
int WordWaffleFinder<T>::findFirstWordRow()
{
    this->mNowTall = 0;
    int prevTall = -1;
    const TrieNode **rowNodes = this->mRowNodes[0], *colBranch;
	const TrieNode  *rowNode = rowNodes[-1]->getFirstChild();
    while (rowNode != NULL) {
...
template <typename T>
int WordWaffleFinder<T>::findEvnRowWantingOddTotal(int haveTall)
{
    assert(haveTall % 2 == 1);
 ...
 */
template <typename T>
int WordWaffleFinder<T>::findEvnRowWantingEvnTotal(int haveTall)
{
    assert(haveTall % 2 == 1);
    this->mNowTall = haveTall;
	return findNextEvnRow(haveTall);
...
/** Having an odd number of rows (haveTall), try to add another row,
  ...
  */
template <typename T>
int WordWaffleFinder<T>::findNextEvnRow(int haveTall)
{
	...
	const TrieNode **rowNodes = this->mRowNodes[haveTall], *colBranch;
	const TrieNode  *rowNode  = rowNodes[-1]->getBranchAtIndex(this->rowCharIndex(colLetter));
    while (rowNode != NULL) {
        int             col = rowNode->getDepth() - 1;
        const char *rowWord = rowNode->getFirstWord();
        // For each vertical stem (partial column) that would result from adding this word
		// as the next row, is there a whole word of length wantTall that could complete it?
		// If yes (IFF the implied colNode is non-null), continue; else break and get next word.
        for (;;) {
...
*/
template <typename T>
int WordWaffleFinder<T>::findOddRowWantingEvnTotal(int haveTall)
{
	assert(haveTall % 2 == 0);
	this->mNowTall = haveTall;
	if (0 < haveTall && this->mWantArea <= WordRectSearchMgr<T>::getFoundArea()) {
 ...
 */
template <typename T>
int WordWaffleFinder<T>::findOddRowWantingOddTotal(int haveTall)
{
	assert(haveTall % 2 == 0);
	this->mNowTall = haveTall;
	if (0 < haveTall && this->mWantArea <= WordRectSearchMgr<T>::getFoundArea()) {
...
/** Having an even number of rows (haveTall), try to add another row
  ...
  */
template <typename T>
int WordWaffleFinder<T>::findNextOddRow(int haveTall)
{
	if (this->mMaxTall < haveTall) {
		this->mMaxTall = haveTall;
	}
