...
void WordRectSearchExec::setOptions(unsigned int managerFlags, unsigned int finderOptions
    , uint minWordLength, uint maxWordLength, int verbosity)
{
	mManagerFlags   = managerFlags;
	mAscending		= (managerFlags & eIncreasingSize)	 > 0 ? true : false;
	mFindLattices	= (managerFlags & eFindLattices)	 > 0 ? true : false;
	mFindWaffles	= (managerFlags & eFindWaffles)		 > 0 ? true : false;
	mOnlyOddDims	= (managerFlags & eOnlyOddDims)		 > 0 ? true : false;
	mSingleThreaded = (managerFlags & eSingleThreaded)	 > 0 ? true : false;
	mUseTrixNodes   = (managerFlags & eTransformIndexes) > 0 ? true : false;
    mFinderOptions  = finderOptions;
...
int WordRectSearchExec::startupSearchManager(const char *dictFile, uint minCharCount
    , uint minArea, uint minTall, uint maxTall, uint maxArea)
{   
    CharFreqMap charFreqMap(dictFile, mMinWordLength, mMaxWordLength, minCharCount); 
    const CharMap::SubType defaultType = charFreqMap.initCountsAndDefaultType(mMinWordLength, mMaxWordLength, mVerbosity);
	...
	mBaseTries = new WordTrie<CharMap>*[mMaxWordLength+1];
	...
	for (uint wordLen = 1; wordLen <= mMaxWordLength; wordLen++)
		mBaseTries[wordLen] = new WordTrie<CharMap>(charMap, wordLen);
	...
	case CharMap::eIdentityCharMap:
		{
			...
			assert(dynamic_cast<const IdentCharMap *>(&identCharMap) != NULL);
	...
	case CharMap::eCompactCharMap:
		{
			...
			assert(dynamic_cast<const CompactCharMap *>(&compactCharMap) != NULL);
	...
	case CharMap::eFreqFirstCharMap:
		{
			...
			assert(dynamic_cast<const FreqFirstCharMap *>(&freqFirstCharMap) != NULL);	// TODO: remove
...
uint WordRectSearchExec::initFromSortedDictionaryFile(const char *fname, const T &charMap
	, WordTrie<T> *wordTries[], WordMap *wordMaps, uint minWordLength, uint maxWordLength)
{   
	...
	TrieNode *prevNode[sMaxWordLength] = { NULL, };
	uint  length, numWords = 0;
    uint  lastMapIdx = charMap.targetOffset() + charMap.targetSize() - 1;
	char  line[CharFreqMap::sBufSize];
    const char *saveWord;
	while (fgets(line, BUFSIZE, fi))  {
		...
		// This allocate this word's memory for the life of the program, i.e.,
		// it is free'd only on program exit, though TrieNode.mWord could be
		// construed as its unique owner, and everyone else only a borrower.
		saveWord = strdup_safe(line);		
		numWords++;
		prevNode[length] = wordTries[length]->insertWord(saveWord, prevNode[length]);
		if (wordMaps != NULL) {
			assert(length > 0);
