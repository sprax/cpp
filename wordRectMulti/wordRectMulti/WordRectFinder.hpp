...
const int MAXLEN  = 128;			// Maximum word length  // TODO: Remove
const int BUFSIZE = MAXLEN + 2; // extra room for terminating NULL, sentinel, or root node at index -1

/**
 *
 *   Examples of Word Rectangle:
 *		5x4 Word Rect:		4x4 Word Rect:		4x4 Symmetric Square, or just "Word Square":
...
public:
    ...
    // Finder options as bit flags
    typedef enum { 
        eFindAll    = 1,    // Find ALL word rects for each size (slower)
        eUseMaps    = 2,
        eUseAltAlg    = 4,
    } FinderOptions;

	WordRectFinder(int wide, int tall, unsigned int options, WordTrie<T> *wordTries[], const WordMap *maps=NULL);
    ...
    inline int          getNumFound()   const   { return mNumWordRects; }
    ...
    virtual void printWordRectInProgress(time_t timeNow)    const
    {
        ...
        printf("Trying %2d * %2d & found %2d rows after %2d seconds in thread %d (SS/WR %d / %d):\n", wantWide, getTall()
            , haveTall, (int)(timeNow - mStartTime), mId, mNumSymSquares, mNumWordRects);
...
protected:
	const WordTrie<T>  &mRowTrie;
	const WordTrie<T>  &mColTrie;
	const int           mWantWide;
	const int           mWantTall;
	...
	int                 mMaxTall;
    int                 mNowTall;
	int					mNumWordRects;			// Number of wantWide X wantTall word rects found
	...
	const TrieNode   ***mRowNodes, **mRowNodesMem;
	const TrieNode   ***mColNodes, **mColNodesMem;
